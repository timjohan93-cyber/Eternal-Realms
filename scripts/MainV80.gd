...
# Compatible with CharacterBody2D player.
...
func create_v80_player_visual() -> void:
	...
	# Player is a CharacterBody2D in the current project. We add the visual as a child
	# instead of trying to cast/replace the player node itself.
	v80_player_visual_root = Node2D.new()
	v80_player_visual_root.name = "V80SharedPlayerVisual"
	v80_player_visual_root.z_index = 80
...
func build_v80_class_visual(root: Node2D, class_id: String, menu_scale: bool = false) -> void:
	...
	add_v80_poly(root, PackedVector2Array([Vector2(-22, 24), Vector2(22, 24), Vector2(34, 30), Vector2(-34, 30)]), Color(0, 0, 0, 0.38), Vector2.ZERO)
